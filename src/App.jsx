...
const callPopTactic = async (message) => {
  const res = await fetch("/api/pop-tactic", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ message }),
  });
  if (!res.ok) throw new Error(`Pop tactique ${res.status}`);
...
/* ─── SPARRING ─── */
function SparringPage({ club, saison, joueuses, evals, matches, chatHistory, reloadChat, coachName, allMatches, onOpenTactic }) {
  const [input, setInput] = useState("");
  const [loading, setLoading] = useState(false);
  const [tacticLoading, setTacticLoading] = useState(null);
          ...
          <div>
            ...
            {h.role==="assistant" && onOpenTactic && (
              <button
                ...
                disabled={tacticLoading===i}
                onClick={async()=>{
                  setTacticLoading(i);
                  try { const data = await callPopTactic(h.content); onOpenTactic(data); }
                  catch(e) { alert(`Erreur: ${e.message}`); }
                  setTacticLoading(null);
                }}
              >{tacticLoading===i ? "⏳..." : "🏀 Ouvrir dans l'éditeur"}</button>
            )}
          </div>
        </div>)}
...
export default function App() {
  ...
  const NAV = [
    { group:"IA", items:[{id:"sparring",icon:"🧠",label:"Sparring Partner"},{id:"gameplan",icon:"🎯",label:"Plan de match"},{id:"training",icon:"⚡",label:"Entraînement"},{id:"tactique",icon:"🏀",label:"Éditeur tactique"}]},
    ...
    <div className="app">
      ...
      <div className="main">
        ...
        <main className="content">
          {page==="sparring" && <SparringPage club={club} saison={saison} joueuses={joueuses} evals={evals} matches={matches} chatHistory={chatHistory} reloadChat={reloadChat} coachName={session.coachName} allMatches={allMatches} onOpenTactic={(data)=>{ setTacticData(data); navigate("tactique"); }}/>}
          ...
          {page==="tactique" && (
            <div style={{height:"calc(100vh - 52px)",margin:"-24px"}}>
              <TacticalBoard
                initData={tacticData}
                onClose={()=>{ setTacticData(null); navigate("sparring"); }}
              />
            </div>
          )}
        </main>
